    ...
    public class AutoUpdater
    {
        ...
        public static async Task<bool> CheckForDMMUpdate(CancellationTokenSource cancellationToken)
        {
            ...
            try
            {
                ...
                {
                    ChangelogBox notification = new ChangelogBox(release, Global.i18n.GetTranslation("Diva Mod Manager"), $"{Global.i18n.GetTranslation("A new version of Diva Mod Manager is available")} (v{onlineVersion})!", null, false);
                    ...
                    if (notification.YesNo)
                    {
                        ...
                        MessageBox.Show($"{Global.i18n.GetTranslation("Finished downloading")} {fileName}!\n{Global.i18n.GetTranslation("Diva Mod Manager will now restart.")}", Global.i18n.GetTranslation("Notification"), MessageBoxButton.OK);
                        // Update DMM
                        ...
                        {
                            MessageBox.Show($"{Global.i18n.GetTranslation("Error parsing")} {onlineVersion}!\n{Global.i18n.GetTranslation("Cancelling update.")}", Global.i18n.GetTranslation("Notification"), MessageBoxButton.OK);
                            return false;
                        }
                        ...
                        Global.logger.WriteLine(Global.i18n.GetTranslation("Update for Diva Mod Manager cancelled."), LoggerType.Info);
                }
                else
                    Global.logger.WriteLine(Global.i18n.GetTranslation("No update for Diva Mod Manager available."), LoggerType.Info);
            }
            catch (Exception ex)
            {
        ...
        private static async Task DownloadDMM(string uri, string fileName, string version, Progress<DownloadProgress> progress, CancellationTokenSource cancellationToken)
        {
            try
            {
                ...
                progressBox.progressText.Text = $"{Global.i18n.GetTranslation("Downloading")} {fileName}";
                progressBox.Title = Global.i18n.GetTranslation("Diva Mod Manager Update Progress");
            ...
            {
                Console.WriteLine($"{Global.i18n.GetTranslation("[ERROR] Error whilst downloading")} {fileName} {e.Message}");
                if (progressBox != null)
                {
                    progressBox.finished = true;
                    progressBox.Close();
